...
const GAS_URL = 'https://script.google.com/macros/s/AKfycbzrKB5V-ymYKZs2KRWLxt0aW4MA2ML5NI2AuGIO12iYCjdujCL84ilXz2VaTQw2SQBs/exec';
const API_TOKEN = 's3cr3t-K9X';
...
async function apiGet(action, params = {}) {
  ...
  try {
    ...
    throw new Error(`apiGet(${action}) returned non-JSON: ${text.slice(0, 200)}`);
  }
}
...
async function apiPost(action, body = {}) {
  ...
  try {
    ...
    throw new Error(`apiPost(${action}) returned non-JSON: ${text.slice(0, 200)}`);
  }
}
...
// current stats -> Roblox + store via GAS
  ...
  try {
    ...
    if (res && res.ok) {
      ...
      if (game) {
        ...
        }).catch(e => console.warn('insert_player_stats_v3', e));
        ...
        }).catch(e => console.warn('upsert_daily_record_v3', e));
        ...
        if (Array.isArray(peak) && peak.length) {
          ...
          if (elements.peakDate) {
            elements.peakDate.textContent = new Date(peak[0].recorded_at).toLocaleDateString(
...
async function calculateFavoritesGrowth(elements, currentFavorites) {
  ...
  try {
    ...
    const data = await apiGet('get_player_stats_since_v3', { since });

    if (Array.isArray(data) && data.length) {
      const base = Number(data[0].favorites || 1);
...
async function loadHistoricalData(elements) {
  try {
    ...
    const labels = data.map(d =>
      new Date(d.label).toLocaleTimeString(
        currentLang === 'en' ? 'en-US' : 'ru-RU',
...
async function loadWeeklyComparison(elements) {
  try {
    ...
    console.error('loadWeeklyComparison', e);
  }
}
...
async function loadWeekdayDistribution(elements) {
  try {
    const data = await apiGet('get_weekday_distribution_v3', { weeks: 6 });
    if (!Array.isArray(data)) return;
    ...
    console.error('loadWeekdayDistribution', e);
  }
}
...
async function loadPeakHours(elements) {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length) {
      elements.peakHours.innerHTML = data.slice(0, 5).map((item, idx) => {
        ...
        const timeLabel = hour
          ? (isNaN(Date.parse(hour))
              ? new Date().toLocaleTimeString(currentLang === 'en' ? 'en-US' : 'ru-RU', { hour: '2-digit', minute: '2-digit' })
              : new Date(hour).toLocaleTimeString(currentLang === 'en' ? 'en-US' : 'ru-RU', { hour: '2-digit', minute: '2-digit' }))
          : '-';
    ...
    if (elements?.peakHours) elements.peakHours.innerHTML = '<div class="text-gray-500">Error</div>';
  }
}
...
async function loadRecords(elements) {
  ...
  try {
    const data = await apiGet('get_records_v3');
    ...
    if (Array.isArray(data) && data.length) {
      elements.recordsTable.innerHTML = data.map((record, index) => {
        ...
        return `<tr class="record-${record.record_type}">
